...
//    private LibraryExpression libForConstants = null;
    ...
    private String libIdString(Expression libraryExpression) {
        if (libraryExpression instanceof NamedExpression namedExpression)
            return namedExpression.getLibIDName();
        return null;
    }
    ...
    public Object evaluate(Context context) throws ReturnException, InterruptedException, VarNotFoundException {
        ...
        for (Expression libraryExpression : libExpressions) { //libraryExpressions) {
            ...
            library = Runtime.loadedLibraries.get(libID); // searches using the source code version of the name
            // e.g. graphics - in this case a function call
            if (library == null)
                try {
